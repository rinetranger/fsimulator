...
const salesVolumeSelector = selector({
  key: "salesVolume",
  get: ({ get }) => {
...
describe("salesVolumeSelector", () => {
    it("should return the correct amtArray based on the version", () => {
      const { result } = renderHook(() => useRecoilValue(salesVolumeSelector), {
        wrapper: RecoilRoot,
      });
  
      const expectedAmtArray = [
        110084,
        134980,
        156733,
        255791,
      ...
      expect(result.current).toEqual(expectedAmtArray);
    });
  });
